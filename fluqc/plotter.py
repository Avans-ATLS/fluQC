...
class Plots:
    ...
    def cov_histogram(self) -> dict[str: Figure]:
        ...
        for s in df["Sample"].unique():
            ...
            fig = make_subplots(
                rows=1, cols=len(df["Segment"].unique()),
    ...
    def readlengths(self) -> dict[str: Figure]:
        ...
        for s in df["Sample"].unique():
            ...
            fig.add_trace(
                Scatter(
                    x=list(self.segment_lengths.keys()), y=list(self.segment_lengths.values()),
                    mode='markers', fillcolor=self.colors['h1'], name='Expected segment size'                  
                )
            )
            fig.update_layout(
                title=f'Segment coverage: {s}',
